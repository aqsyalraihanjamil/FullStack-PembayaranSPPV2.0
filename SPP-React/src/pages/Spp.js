...
export default class Spp extends Component {
  ...
  render() {
    const { showingEdit, spp } = this.state
    return (
      <div>
        <Sidebar />
        <div>
                    ...
                    <div className="table-row font-body font-medium text-xl w-full">
                      <td colSpan="5">
                        <div className='p-2 flex items-center justify-center h-full'>
                          <p className='text-grey-activities text-opacity-80 text-lg'>Showing entries of {this.state.spp.length} from {this.state.spp.length}</p>
                    ...
                    <div className="space-y-4 h-full">
                      {this.state.action === "update" && (
                        <div className='font-body space-y-2'>
                          ...
                          <input className=' block relative w-full border border-gray-300 bg-gray-50 text-gray-900 placeholder-grey-666 p-2 h-10 rounded-md focus:outline-none focus:border-purple-light' disabled placeholder='ID' type="text" value={this.state.id_spp} onChange={ev => this.setState({ id_spp: ev.target.value })} required />
                        </div>
                      )}
                      <div className='font-body space-y-2'>
                        <label className='text-xl font-medium'>Angkatan</label>
                        <input className=' block relative w-full border border-gray-300 bg-gray-50 text-gray-900 placeholder-grey-666 p-2 h-10 rounded-md focus:outline-none focus:border-purple-light' placeholder='Angkatan' type="text" value={this.state.angkatan} onChange={ev => this.setState({ angkatan: ev.target.value })} required />
                      </div>
                      <div className='font-body space-y-2'>
                        <label className='text-xl font-medium'>Tahun</label>
                        <input className=' block relative w-full border border-gray-300 bg-gray-50 text-gray-900 placeholder-grey-666 p-2 h-10 rounded-md focus:outline-none focus:border-purple-light' placeholder='Tahun' type="text" value={this.state.tahun} onChange={ev => this.setState({ tahun: ev.target.value })} required />
                      </div>
                      <div className='font-body space-y-2'>
                        <label className='text-xl font-medium'>Nominal</label>
                        <input className=' block relative w-full border border-gray-300 bg-gray-50 text-gray-900 placeholder-grey-666 p-2 h-10 rounded-md focus:outline-none focus:border-purple-light' placeholder='Nominal' type="text" value={this.state.nominal} onChange={ev => this.setState({ nominal: ev.target.value })} required />
                      </div>
