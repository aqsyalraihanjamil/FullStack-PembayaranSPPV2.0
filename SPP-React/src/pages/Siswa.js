...
export default class Siswa extends Component {
  ...
  getDetailKelas = (id) => {
    ...
    axios
      .get(url, this.headerConfig())
      .then((response) => {
        this.setState({ tunggakan: response.data.angkatan });
        ...
        <div className="xl:pl-76 h-screen xl:pt-20 pt-16 w-screen bg-grey-eee overflow-hidden ">
          <div className="p-8 h-full">
            <div className="flex gap-4">
              ...
              <div className="w-1/3 flex gap-4 ml-auto mr-0 ">
                {/* Go to kelas page */}
                        ...
                        <div className="table-row font-body text-lg 2xl:text-xl" key={index}>
                          ...
                          <div className="table-cell align-middle border-b-2 border-solid border-grey-eee  overflow-hidden">
                            <div className="lg:flex w-full h-full items-center gap-4  hidden">
                              <button
                                className="h-12 bg-purple-verylight w-1/3 shadow-sm rounded-full items-center flex justify-center"
                                onClick={() => {
                              ...
                              >
                                {" "}
                                <p className="w-1/2 text-center font-medium">Detail</p> <Detail className="h-1/2 w-1/6 xl:w-1/5" />{" "}
                              </button>
                              <button
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="font-body space-y-2">
                      ...
                      <select
                        ...
                        onChange={(ev) => {
                          this.setState({ id_kelas: ev.target.value },()=>{this.getDetailKelas(this.state.id_kelas)});
                        }}
                    ...
                    <div className="font-body space-y-2">
                      ...
                      >
                        ...
                        {/* get Id kelas to match with the same angkatan */}
                         {spp && spp.flatMap(item => (item.angkatan === this.state.tunggakan) ? <option value={item.id_spp}>{item.id_spp} - Rp. {item.nominal} ( Angkatan {item.angkatan} )</option> : null)}
                        {/* <option value={item.id_spp}>{item.id_spp} - Rp. {item.nominal} ( Angkatan {item.angkatan} )</option>  */}
                      </select>
                    </div>
                    {this.state.action === "update" && this.state.uploadFile === false ? (
                      <button className="block relative bg-purple-light text-white text-lg font-medium w-full p-2 rounded-xl" onClick={() => this.setState({ uploadFile: true })}>
                        Change Image
                      </button>
                    ) : (
                      <div>
